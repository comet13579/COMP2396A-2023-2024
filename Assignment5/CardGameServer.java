...
public class CardGameServer {
    ...
    public void start(int port) {
        ...
        try {
            ...
            println("Starts up the server at localhost:" + serverSocket.getLocalPort());
            serverUp = true;
            while (serverUp) {
     ...
     */
    private synchronized void parseMessage(Socket clientSocket, CardGameMessage message) {
        ...
        case CardGameMessage.MOVE:
            println("Broadcasts a \"MOVE\" message from " + clientSocket.getRemoteSocketAddress());
            // broadcast the MOVE message to all clients
            broadcastMessage(message);
            break;
        case CardGameMessage.MSG:
            println("Broadcasts a user message from " + clientSocket.getRemoteSocketAddress());
            ...
            for (int i = 0; i < maxNumOfPlayers; i++) {
                if (clientSockets[i] != null && clientOutputStreams[i] != null) {
                    try {
                        ...
                        println("Error in broadcasting a message to the client at " + clientSockets[i].getRemoteSocketAddress());
                        if (!silentMode)
                            ex.printStackTrace();
                    }
                }
            ...
            for (int i = 0; i < maxNumOfPlayers; i++) {
                if (clientSockets[i] == null) {
                    try {
                        ...
                        println("Error in establishing a connection with a client at " + clientSocket.getRemoteSocketAddress());
                        if (!silentMode)
                            ex.printStackTrace();
                    }
                    break;
        ...
        if (numOfPlayers > 0) {
            ...
            for (int i = 0; i < maxNumOfPlayers; i++) {
                if (clientSockets[i] == clientSocket) {
                    ...
                    println(name + " (" + clientSocket.getRemoteSocketAddress() + ") leaves the game.");

                    String remoteAddress = clientSocket.getRemoteSocketAddress().toString();

                    // broadcasts a message about the leaving of this player
                    broadcastMessage(new CardGameMessage(CardGameMessage.QUIT, i, remoteAddress));
                    break;
                }
            }
        }
        ...
        if (numOfPlayers > 0) {
            ...
            for (int i = 0; i < maxNumOfPlayers; i++) {
                if (clientSockets[i] == clientSocket) {
                    ...
                    println(name + " (" + clientSocket.getRemoteSocketAddress() + ") joins the game.");

                    // broadcasts a message about this player joining the game
                    broadcastMessage(new CardGameMessage(CardGameMessage.JOIN, i, name));
                    break;
                } 
            }
        }
    ...
    private synchronized void setReadyState(Socket clientSocket) {
        if (numOfPlayers > 0) {
            ...
            for (int i = 0; i < maxNumOfPlayers; i++) {
                if (clientSockets[i] == clientSocket) {
                    ...
                    println(clientNames[i] + " (" + clientSocket.getRemoteSocketAddress() + " ) is ready for the next game.");
                    broadcastMessage(new CardGameMessage(CardGameMessage.READY, i, null));
                    break;
                }
            }
        }
        ...
        if (numOfPlayers == maxNumOfPlayers) {
            ...
            broadcastMessage(new CardGameMessage(CardGameMessage.START, -1, deck));
        }
    }
     ...
     */
    public Deck createDeck() { return new Deck(); }
    ...
    private synchronized void broadcastUserMessage(Socket clientSocket, String msg) {
        if (numOfPlayers > 0) {
            ...
            for (int i = 0; i < maxNumOfPlayers; i++) {
                if (clientSockets[i] == clientSocket) {
                    String longMsg = clientNames[i] + " (" + clientSocket.getRemoteSocketAddress() + "): " + msg;
                    broadcastMessage(new CardGameMessage(CardGameMessage.MSG, i, longMsg));
                    break;
                }
            }
        }
    ...
    private class ClientHandler implements Runnable {
        ...
        public ClientHandler(Socket clientSocket) {
            ...
            try {
                ...
                println("Error in creating an ObjectInputStream for the client at " + clientSocket.getRemoteSocketAddress());
                if (!silentMode)
                    ex.printStackTrace();
            }
        } // constructor
        ...
        public void run() {
            ...
            try {
                ...
                while ((message = (CardGameMessage)oistream.readObject()) != null) {
                    println("Message received from " + clientSocket.getRemoteSocketAddress());
                    parseMessage(clientSocket, message);
                } // close while
            } catch (Exception ex) {
                println("Error in receiving messages from the client at " + clientSocket.getRemoteSocketAddress());
                if (!silentMode)
                    ex.printStackTrace();
    ...
    private class ClientHandler2 implements Runnable {
         ...
         *            the socket connection to the client
         */
        public ClientHandler2(Socket clientSocket) { this.clientSocket = clientSocket; } // constructor
        ...
        public void run() {
            try {
                ...
                oostream.writeObject(new CardGameMessage(CardGameMessage.FULL, -1, null));
                oostream.flush();
            } catch (Exception ex) {
                println("Error in sending a FULL message to the client at " + clientSocket.getRemoteSocketAddress());
                if (!silentMode)
                    ex.printStackTrace();
            }
            ...
            try {
                ...
                println("Error in sleeping before closing the client socket at " + clientSocket.getRemoteSocketAddress());
                if (!silentMode)
                    ex.printStackTrace();
            }
            ...
            try {
                ...
                println("Error in closing the client socket at " + clientSocket.getRemoteSocketAddress());
                if (!silentMode)
                    ex.printStackTrace();
            }
        } // run
